...
class TB:
    def __init__(self, dut):
        self.dut = dut
        # Get width from one of the ALU's operand ports (assumed to be 'A')
        ...
        self.table = Table(title="ALU Test Results")
    ...
    def performance_model(self, A, B, control):
        ...
        # Determine expected result and flags based on the selected operation:
        if control == 0:  # Bitwise AND
            ...
            expected_co = 0
            expected_ovf = 0
        elif control == 2:  # Subtraction (A - B)
            op = "SUB"
            result = A - B
            res = result & mask
            # Compute carry out by performing two's complement addition: A + (~B + 1)
            sum_val = A_u + (((~B_u)+ 1) & mask)
            expected_co = 1 if sum_val > mask else 0
            # Compute signed result from the truncated value
            signed_res = res - (1 << width) if res >= (1 << (width - 1)) else res
            # Overflow occurs when the signs of A and B differ and the sign of the result differs from A
            expected_ovf = 1 if ((A < 0) != (B < 0)) and ((signed_res < 0) != (A < 0)) else 0
        elif control == 3:  # Addition (A + B)
            op = "ADD"
            result = A + B
            res = result & mask
            sum_val = A_u + B_u
            expected_co = 1 if sum_val > mask else 0
            signed_res = res - (1 << width) if res >= (1 << (width - 1)) else res
            # Overflow occurs when A and B have the same sign but the result's sign differs from A
            expected_ovf = 1 if ((A < 0) == (B < 0)) and ((signed_res < 0) != (A < 0)) else 0
        else:
            op = "UNK"
            res = 0
        ...
        # Negative flag: 1 if the most-significant bit is 1; Zero flag: 1 if the result is zero
    ...
    async def run_random_test(self, total_test_no):
        for _ in range(total_test_no):
            # Randomly select A control value (0: AND, 1: OR, 2: SUB, 3: ADD)
            control = random.randint(0, 3)
            # For arithmetic operations (SUB, ADD) use signed range; for logic operations use full unsigned range.
            ...
            # Wait A short time (combinational ALU; no clock required)
...
async def alu_test(dut):
    """
    Top-level cocotb test for the ALU module.
    Assumes the ALU has the following ports:
        Inputs:  A, B (W-bit), control (2-bit)
